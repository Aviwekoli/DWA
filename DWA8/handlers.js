 ...
 * @prop {object} book - The book data
 * @prop {string} book.id - The unique ID of the book
 * @prop {string} book.title - The title of the book
 * @prop {string} book.author - The author of the book
 * @prop {string} book.image - The URL the book's image
 * @returns {HTMLElement} - The create preview button element
 */
export const createBookPreview = () => {
    return {
        generateHTML: ({id, image, title, author}) => {
            if (!id || !image || !title || !author || !authors[author]) {
                throw new Error('The book data provided is invalid');
            }
    ...
    return {
        render: (booksToShow) => {
            ...
            for (const book of books.slice((page -1) * BOOKS_PER_PAGE, (page) * BOOKS_PER_PAGE)) {
                const element = createBookPreview().generateHTML(book)
